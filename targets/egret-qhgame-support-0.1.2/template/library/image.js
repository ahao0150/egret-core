        ...
        if (path.isRemotePath(imageSrc)) { //判断是本地加载还是网络加载
            if (!needCache(root, url)) {
                ...
                if (fs.existsSync(fullname)) {
                    ...
                    return loadImage(path.getQhUserPath(fullname), scale9Grid);
                } else {
...
function download(url, target) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const file_target = path.getQhUserPath(target);
        qh.downloadFile({
            url: url,
            ...
            success: (v) => {
                if (v.statusCode >= 400) {
                    try {
                        QHFS.accessSync(file_target);
                        QHFS.unlinkSync(file_target);
                    } catch (e) {

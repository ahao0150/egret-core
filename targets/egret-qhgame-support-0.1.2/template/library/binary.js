            ...
            if (path.isRemotePath(xhrURL)) {
                if (needCache(xhrURL)) {
                    ...
                    if (fs.existsSync(targetFilename)) {
                        //缓存命中
                        let data = QHFS.readFileSync(path.getQhUserPath(targetFilename));
                        resolve(data);
                    } else {
                        loadBinary(xhrURL).then((content) => {
                            ...
                            if (needRead) {
                                content = QHFS.readFileSync(path.getQhUserPath(targetFilename));
                            }
                            resolve(content);
                        }).catch((e) => {
                            reject(e);
                ...
                try {
                    const content = QHFS.readFileSync(xhrURL);
                    resolve(content);
                } catch (e) {
                    resolve(null);
                }
...
function needReadFile() {
    if (!qhSystemInfo) {
        qhSystemInfo = qh.getSystemInfoSync();
    }
    let sdkVersion = qhSystemInfo.SDKVersion;
    let platform = qhSystemInfo.system.split(" ").shift();

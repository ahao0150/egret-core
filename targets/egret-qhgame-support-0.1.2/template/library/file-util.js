 ...
 * 封装微信小游戏的文件系统
 */
const qhFs = qh.getFileSystemManager();
const QH_ROOT = qh.env.USER_DATA_PATH + "/";

function walkFile(dirname, callback) {
    const files = qhFs.readdirSync(dirname)
    for (let f of files) {
        const file = dirname + "/" + f;
        const stat = qhFs.statSync(file);
        if (stat.isDirectory()) {
...
const fs = {
    ...
    remove: (dirname) => {
        if (!fs.existsSync(dirname))
            return;
        const globalDirname = QH_ROOT + dirname;
        walkFile(globalDirname, (file) => {
            qhFs.unlinkSync(file);
            let p = file.replace(QH_ROOT, "");
            p = path.normailze(p);
            if (fs_cache[p]) {
                fs_cache[p] = 0;
            }
    ...
    writeSync: (p, content) => {
        p = path.normailze(p);
        fs_cache[p] = 1;
        qhFs.writeFileSync(QH_ROOT + p, content);
    },

    readSync: (p, format) => {
        format = format || 'utf-8';
        return qhFs.readFileSync(QH_ROOT + p, format);
    },
        ...
        if (!fs.existsSync(p)) {
            ...
            for (let i = 0; i < dirs.length; i++) {
                ...
                if (!fs.existsSync(current)) {
                    ...
                    fs_cache[p] = 1;
                    qhFs.mkdirSync(QH_ROOT + current)
    ...
    unzip: (zipFilePath, targetPath) => {
        zipFilePath = QH_ROOT + zipFilePath;
        targetPath = QH_ROOT + targetPath;
